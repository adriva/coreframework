...
namespace Adriva.Extensions.Caching.SqlServer
{
    ...
    public class SqlServerCache : IDistributedCache, ICache
    {
        ...
        {
            if ((DateTimeOffset.UtcNow - this.LastExpirationScan) > ExpiredItemsDeletionInterval)
            {
                lock (this.Mutex)
                {
                    if ((DateTimeOffset.UtcNow - this.LastExpirationScan) > ExpiredItemsDeletionInterval)
                    {
                        this.LastExpirationScan = DateTimeOffset.UtcNow;
        ...
        public async ValueTask<bool> ClearAsync()
        {
            try
            {
                await this.DbOperations.DeleteAllItemsAsync();
                return true;
            }
            catch
            {
