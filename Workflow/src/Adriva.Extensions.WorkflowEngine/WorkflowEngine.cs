...
namespace Adriva.Extensions.WorkflowEngine
{
    internal sealed class WorkflowEngine : IWorkflowEngine
    {
        ...
        public WorkflowEngine(IServiceProvider serviceProvider, string name, IOptionsSnapshot<WorkflowEngineOptions> optionsAccessor)
        {
        ...
        private void ValidateWorkflow(Workflow workflow, InputParameter[] inputParameters)
        {
            ...
            while (0 < stepQueue.Count)
            {
                ...
                currentStep.Properties = currentStep.Properties ?? new Dictionary<string, object>();
